...
export const getFeed = query({
  ...
  handler: async (ctx, args) => {
    ...
    const sortedFeed = Array.from(uniqueMap.values())
      .sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0)) // Newest first, fallback to 0 if undefined
      .slice(0, limit);

    return {
      items: sortedFeed,
      nextCursor: sortedFeed.length > 0 ? (sortedFeed[sortedFeed.length - 1].createdAt || 0) : null,
...
async function getLocalTrends(ctx: any, geohash: string, limit: number, sevenDaysAgo: number, cursor: number) {
  ...
  return {
    items: finalContent,
    nextCursor: finalContent.length > 0 ? (finalContent[finalContent.length - 1].createdAt || 0) : null,
